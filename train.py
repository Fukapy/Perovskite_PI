...
def makemodel(model_name, max_depth,max_leaf_nodes, n_estimators, min_samples_split, min_samples_leaf, dim, n_mid, activation, solver, lr, epoch, kernel_func, polydeg, epsilon):
...
def objective_variable(model_name, X_train, y_train,valid_ratio_in_train):
    ...
    def objective(trial):
        ...
        if model_name == "RF":
            ...
            # max_depth = trial.suggest_int('max_depth', 50, 50, step=50)
            # max_leaf_nodes = trial.suggest_int('max_leaf_nodes',10, 10, step=10)
            ...
            # max_depth = None 3/7
            # max_leaf_nodes = trial.suggest_int('max_leaf_nodes',50, 500, step=50)
            n_estimators =  trial.suggest_int('n_estimators',10, 300, step=10)
            min_samples_split = trial.suggest_int('min_samples_split',
                                                  2,20)
            min_samples_leaf = trial.suggest_int('min_samples_leaf',
            ...
            dim = trial.suggest_categorical('dim',
                                           [16,32,64,128,256])
            n_mid = trial.suggest_int('n_mid',
                                      1, 5)
            ...
            kernel_func = trial.suggest_categorical('kernel_func',
                                                    ['linear','poly','rbf', 'sigmoid'])
            polydeg = trial.suggest_int('polydeg',
                                        1, 10)
            epsilon = trial.suggest_discrete_uniform('epsilon',
                                                    0,5,0.1)
            
            regr = SVR(kernel = kernel_func,
                       degree = polydeg,
                       epsilon = epsilon)
